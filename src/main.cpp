...
int main() {
  ...
  while (getline(in_map_, line)) {
  	...
  	pathPlan.updateWaypoints(x, y, s, d_x, d_y);
  }

  h.onMessage([&lane,&ref_vel,&projecting_dist, &pathPlan](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode) {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
            ...
            pathPlan.createTrajectory(next_x_vals,next_y_vals, lane,ref_vel,projecting_dist );
